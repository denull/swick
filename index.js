...
class UnmountedComponent extends Component {
  ...
  mount() {
    const comp = this.parent[this.childName] = new Micro.components[this.compName](this.data);
    comp.mount(this.el);
    return comp;
  }
...
Micro.mount = (appEl) => {
  if (!isDOMReady) {
    waitingForDOM.push(Micro.mount.bind(this, appEl));
    return;
  }
  ...
  for (let child of appEl.children) {
    if (child.classList[0]) {
      ...
      if (compName in Micro.components) {
        const props = getDatasetProps(child);
        const comp = new Micro.components[compName](props, props.get('isRendered') ? child : null);
        !props.get('isRendered') && comp.mount(child);
        continue;
      }
    }
    Micro.mount(child); // Scan children recursively
  }
}

document.addEventListener('DOMContentLoaded', () => {
  Micro.registerTemplates(document.getElementById('templates'));
  isDOMReady = true;
